...
var calculateAge = () => {
  ...
  let birth_date = new Date("Dec, 15, 1999");
  let years, months, days, hours, minutes, seconds;
  setInterval(function () {
    var current_date = new Date();
    AGE = current_date.getYear() - birth_date.getYear();
    age_year.innerHTML = pad(AGE);
    age_months.innerHTML = pad(
      Math.abs(current_date.getMonth() - birth_date.getMonth())
    );
    age_days.innerHTML = pad(
      Math.abs(current_date.getDate() - birth_date.getDate())
    );
    age_hours.innerHTML = pad(
      Math.abs(current_date.getHours() - birth_date.getHours())
    );
    age_minutes.innerHTML = pad(
      Math.abs(current_date.getMinutes() - birth_date.getMinutes())
    );
    age_seconds.innerHTML = pad(
      Math.abs(current_date.getSeconds() - birth_date.getSeconds())
    );
  }, 1000);
};
